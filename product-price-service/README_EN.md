...
Run this from `product-price-service`:
...
```text
com.viettel.bccs.price -> com.viettel.bccs.payment
```
...
- Execute `ProductPriceServiceApplication.java` from your IDE.
...
All classes in `com.viettel.bccs.price` are sample code. Use them as examples, then replace them with business-specific names and behavior.
...
```text
product-price-service.sample-order.created
product-price-service.sample-order.dlq
```
