...
Run this from `product-area-service`:
...
```text
com.viettel.bccs.area -> com.viettel.bccs.payment
```
...
- Execute `ProductAreaServiceApplication.java` from your IDE.
...
All classes in `com.viettel.bccs.area` are sample code. Use them as examples, then replace them with business-specific names and behavior.
...
```text
product-area-service.sample-order.created
product-area-service.sample-order.dlq
```
