...
Chạy từ thư mục `product-area-service`:
...
```text
src/main/java/com/viettel/bccs/area  ->  src/main/java/com/viettel/bccs/payment
src/test/java/com/viettel/bccs/area  ->  src/test/java/com/viettel/bccs/payment
```

Sau đó thay tất cả `com.viettel.bccs.area` → `com.viettel.bccs.payment` trong mọi file `.java` và `.yml`.
...
```text
ProductAreaServiceApplication.java  ->  PaymentServiceApplication.java
```
...
bccs:
  sample-order:
    created: product-area-service.sample-order.created
    dlq: product-area-service.sample-order.dlq
...
Mọi class trong `com.viettel.bccs.area` đều là code mẫu. Dùng làm ví dụ, sau đó thay bằng tên và hành vi theo nghiệp vụ.
