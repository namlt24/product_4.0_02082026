...
@NoArgsConstructor
@AllArgsConstructor
@Schema(description = "Request tìm sản phẩm (product_offering) theo danh sách mã đặc tính,"
        + " tuỳ chọn lọc thêm theo danh sách mã sản phẩm")
public class FindProductOfferingByListCodeListSpecCodeRequest implements Serializable {

    @Schema(description = "Danh sách mã sản phẩm cần lọc (product_offering.code) — không truyền thì"
            + " lấy tất cả sản phẩm khớp lstSpecCode", example = "[\"CODE_001\", \"CODE_002\"]")
    ...
    @Schema(description = "Danh sách mã đặc tính cần khớp (product_spec_char.code) — bắt buộc, ít nhất 1 phần tử; "
            + "validate thủ công trong ProductOfferingService (ném BCCS-PRODUCT-VALIDATE-0001 nếu rỗng)"
            + " thay vì Bean Validation",
    ...
    @Schema(description = "ID loại sản phẩm (product_offer_type_id) — mặc định 200 (PRODUCT_CODE) nếu không truyền",
            example = "200")
