...
Run this from `product-policy-service`:
...
```text
com.viettel.bccs.policy -> com.viettel.bccs.payment
```
...
- Execute `ProductPolicyServiceApplication.java` from your IDE.
...
All classes in `com.viettel.bccs.policy` are sample code. Use them as examples, then replace them with business-specific names and behavior.
...
```text
product-policy-service.sample-order.created
product-policy-service.sample-order.dlq
```
