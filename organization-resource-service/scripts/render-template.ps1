...
$placeholderPackageRoot = Join-Path (Join-Path $javaRoot "com.viettel.bccs") "organization"
...
foreach ($file in $files) {
    if ($textExtensions -contains $file.Extension) {
        ...
        $content = $content.Replace("organization-resource-service", $ServiceName)
        $content = $content.Replace("com.viettel.bccs", $BasePackage)
        $content = $content.Replace("organization", $ServicePackage)
        $content = $content.Replace("organization-resource-service", $ServiceName)
        $content = $content.Replace("com.viettel.bccs.organization", $packageName)
        $content = $content.Replace("OrganizationResourceServiceApplication", $applicationClassName)
        $content = $content.Replace("OrganizationResourceServiceApplication", $applicationClassName)
...
$templateApplicationFiles = @(
    (Join-Path $renderedPackageRoot "OrganizationResourceServiceApplication.java"),
    (Join-Path $renderedPackageRoot "OrganizationResourceServiceApplication.java")
)
