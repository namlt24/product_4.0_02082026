 ...
 * (xem ProductOfferPriceEntity) — field không nullable ở DB vẫn cho phép null ở response
 * record (record component không có @NotNull), @Size/@Pattern/@Min/@Max chỉ áp dụng khi
 * giá trị khác null. priceEquipmentId/priceEquipmentTypeId không map trực tiếp cột nào
 * (được set động trong mapper.withPriceInfo từ ProductOfferPriceEntity khác), dùng lại
 * bound của productOfferPriceId/priceTypeId tương ứng.
 */
public record ProductOfferPriceResponse(
        ...
        @Schema(description = "Giá thiết bị khuyến mãi (từ SensorFeeRule/ProductOfferPriceEntity.price)")
        ...
        @Schema(description = "ID loại giá của giá thiết bị khuyến mãi (ProductOfferPriceEntity.priceTypeId)")
        @Min(value = 1, message = "priceEquipmentTypeId phải >= 1")
        @Max(value = 9999999999L, message = "priceEquipmentTypeId vượt quá độ dài cột (precision 10)")
