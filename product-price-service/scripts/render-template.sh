...
PLACEHOLDER_PACKAGE_ROOT="$OUTPUT_PATH/src/main/java/com.viettel.bccs/price"
...
\) -print0 | while IFS= read -r -d '' file; do
  sed -i.bak \
    -e "s|product-price-service|$SERVICE_NAME|g" \
    -e "s|com.viettel.bccs|$BASE_PACKAGE|g" \
    -e "s|price|$SERVICE_PACKAGE|g" \
    -e "s|product-price-service|$SERVICE_NAME|g" \
    -e "s|com.viettel.bccs.price|$PACKAGE_NAME|g" \
    -e "s|ProductPriceServiceApplication|$APPLICATION_CLASS_NAME|g" \
    -e "s|ProductPriceServiceApplication|$APPLICATION_CLASS_NAME|g" \
    "$file"
  rm -f "$file.bak"
done
for template_application_file in \
  "$RENDERED_PACKAGE_ROOT/ProductPriceServiceApplication.java" \
  "$RENDERED_PACKAGE_ROOT/ProductPriceServiceApplication.java"; do
