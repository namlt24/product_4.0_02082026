...
$placeholderPackageRoot = Join-Path (Join-Path $javaRoot "com.viettel.bccs") "area"
...
foreach ($file in $files) {
    if ($textExtensions -contains $file.Extension) {
        ...
        $content = $content.Replace("product-area-service", $ServiceName)
        $content = $content.Replace("com.viettel.bccs", $BasePackage)
        $content = $content.Replace("area", $ServicePackage)
        $content = $content.Replace("product-area-service", $ServiceName)
        $content = $content.Replace("com.viettel.bccs.area", $packageName)
        $content = $content.Replace("ProductAreaServiceApplication", $applicationClassName)
        $content = $content.Replace("ProductAreaServiceApplication", $applicationClassName)
...
$templateApplicationFiles = @(
    (Join-Path $renderedPackageRoot "ProductAreaServiceApplication.java"),
    (Join-Path $renderedPackageRoot "ProductAreaServiceApplication.java")
)
