...
Chạy từ thư mục `organization-resource-service`:
...
```text
src/main/java/com/viettel/bccs/organization  ->  src/main/java/com/viettel/bccs/payment
src/test/java/com/viettel/bccs/organization  ->  src/test/java/com/viettel/bccs/payment
```

Sau đó thay tất cả `com.viettel.bccs.organization` → `com.viettel.bccs.payment` trong mọi file `.java` và `.yml`.
...
```text
OrganizationResourceServiceApplication.java  ->  PaymentServiceApplication.java
```
...
bccs:
  sample-order:
    created: organization-resource-service.sample-order.created
    dlq: organization-resource-service.sample-order.dlq
...
Mọi class trong `com.viettel.bccs.organization` đều là code mẫu. Dùng làm ví dụ, sau đó thay bằng tên và hành vi theo nghiệp vụ.
