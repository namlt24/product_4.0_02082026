...
Run this from `organization-resource-service`:
...
```text
com.viettel.bccs.organization -> com.viettel.bccs.payment
```
...
- Execute `OrganizationResourceServiceApplication.java` from your IDE.
...
All classes in `com.viettel.bccs.organization` are sample code. Use them as examples, then replace them with business-specific names and behavior.
...
```text
organization-resource-service.sample-order.created
organization-resource-service.sample-order.dlq
```
