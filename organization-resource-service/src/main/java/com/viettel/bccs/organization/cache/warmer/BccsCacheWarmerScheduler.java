...
public class BccsCacheWarmerScheduler {
    ...
    @Scheduled(
            initialDelayString = "${bccs.cache.warmer.tick-interval-ms:60000}",
    ...
    private void runTask(BccsCacheWarmTask task) {
        ...
        try {
            ...
            log.info("Đã warm cache '{}' trong {}ms", task.cacheName(), System.currentTimeMillis() - start);
        } catch (Exception ex) {
            log.warn("Warm cache '{}' lỗi: {}", task.cacheName(), ex.getMessage(), ex);
        }
    }
}
