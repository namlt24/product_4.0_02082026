...
@Operation(operationId = "getMapSkipDebtChargeFullInfo",
        ...
        requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                content = @Content(array = @ArraySchema(schema = @Schema(implementation = MapSkipDebtChargesDTO.class)),
                        examples = @ExampleObject(name = "request", value = MapSkipDebtChargesControllerExamples.MAP_SKIP_DEBT_CHARGES_LIST_REQUEST_EXAMPLE))))
@ApiResponses({
        @ApiResponse(responseCode = "200", description = "Thành công",
                content = @Content(schema = @Schema(implementation = StandardResponse.class),
                        examples = @ExampleObject(name = "success", value = MapSkipDebtChargesControllerExamples.MAP_SKIP_DEBT_CHARGES_FULL_LIST_EXAMPLE)))
})
