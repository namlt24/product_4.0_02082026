 ...
 * co the goi NGUOC vao chinh gateway nay (Upstream co baseHost = localhost:{port})
 * voi url_pattern trung cau truc voi path cua 1 endpoint KHAC dang luu trong he
 * thong - do la 1 canh phu thuoc (khong can nguoi dung khai bao gi them, suy
 * ra thang tu du lieu da co). Tu khi engine ho tro sourceType=REQUEST_BODY,
 * trick nay it can thiet hon truoc (da co cach doc truc tiep tu body client),
 * nhung van giu lai cho truong hop thuc su muon 1 endpoint goi lai endpoint khac.
 *
 * Dung de:
...
public class DependencyAnalyzer {
    ...
    public DependencyGraphDto buildGraph() {
        ...
        for (EndpointConfig ep : all) {
            ...
            for (BackendStep step : steps) {
                if (step.getUpstreamService() == null || !isSelfHost(step.getUpstreamService().getBaseHost())) {
                    continue;
                }
    ...
    private boolean isSelfHost(String baseHost) {
        if (baseHost == null || baseHost.isBlank()) {
            return false;
        }
        try {
            URI uri = URI.create(baseHost.trim());
            String hostName = uri.getHost();
            int port = uri.getPort();
            return hostName != null && port == gatewayPort
