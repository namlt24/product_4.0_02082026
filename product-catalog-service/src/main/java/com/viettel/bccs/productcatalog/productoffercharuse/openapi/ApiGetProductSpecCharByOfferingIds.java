...
@Operation(operationId = "getProductSpecCharByOfferingIds",
        ...
        description = "Batch fetch đặc tính sản phẩm cho nhiều offering IDs trong 1 query,"
                + " trả về Map<offeringId, list specChar>")
@ApiResponses({
        @ApiResponse(responseCode = "200", description = "Thành công",
                content = @Content(schema = @Schema(implementation = StandardResponse.class),
                        examples = @ExampleObject(name = "success",
                            value = ProductOfferCharUseControllerExamples.SPEC_CHAR_MAP_EXAMPLE)))})
