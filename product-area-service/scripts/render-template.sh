...
PLACEHOLDER_PACKAGE_ROOT="$OUTPUT_PATH/src/main/java/com.viettel.bccs/area"
...
\) -print0 | while IFS= read -r -d '' file; do
  sed -i.bak \
    -e "s|product-area-service|$SERVICE_NAME|g" \
    -e "s|com.viettel.bccs|$BASE_PACKAGE|g" \
    -e "s|area|$SERVICE_PACKAGE|g" \
    -e "s|product-area-service|$SERVICE_NAME|g" \
    -e "s|com.viettel.bccs.area|$PACKAGE_NAME|g" \
    -e "s|ProductAreaServiceApplication|$APPLICATION_CLASS_NAME|g" \
    -e "s|ProductAreaServiceApplication|$APPLICATION_CLASS_NAME|g" \
    "$file"
  rm -f "$file.bak"
done
for template_application_file in \
  "$RENDERED_PACKAGE_ROOT/ProductAreaServiceApplication.java" \
  "$RENDERED_PACKAGE_ROOT/ProductAreaServiceApplication.java"; do
