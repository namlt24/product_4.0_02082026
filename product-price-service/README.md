...
Chạy từ thư mục `product-price-service`:
...
```text
src/main/java/com/viettel/bccs/price  ->  src/main/java/com/viettel/bccs/payment
src/test/java/com/viettel/bccs/price  ->  src/test/java/com/viettel/bccs/payment
```

Sau đó thay tất cả `com.viettel.bccs.price` → `com.viettel.bccs.payment` trong mọi file `.java` và `.yml`.
...
```text
ProductPriceServiceApplication.java  ->  PaymentServiceApplication.java
```
...
bccs:
  sample-order:
    created: product-price-service.sample-order.created
    dlq: product-price-service.sample-order.dlq
...
Mọi class trong `com.viettel.bccs.price` đều là code mẫu. Dùng làm ví dụ, sau đó thay bằng tên và hành vi theo nghiệp vụ.
