...
@Operation(operationId = "getAreaByCode", summary = "Lấy địa bàn theo mã",
        description = "Tra cứu 1 bản ghi AREA theo AREA_CODE (khoá chính)."
                + " Ví dụ dùng dữ liệu mẫu 'A076' (An Giang) đã có sẵn trong DB.")
@ApiResponses({
        @ApiResponse(responseCode = "200", description = "Thành công",
                content = @Content(schema = @Schema(implementation = StandardResponse.class),
                        examples = @ExampleObject(
                        name = "success",
                        value = AreaControllerExamples.AREA_SINGLE_EXAMPLE))),
