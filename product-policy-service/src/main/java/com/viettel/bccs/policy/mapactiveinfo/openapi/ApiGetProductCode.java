...
@Operation(operationId = "getProductCode",
        ...
        description = "Trả về danh sách gói cước (product code) hợp lệ theo map active info."
                + " Luôn ép kiểu PRODUCT_CODE và checkProductStatus=true, không lọc theo VAS.",
        requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                content = @Content(schema = @Schema(implementation = GetProductCodeRequest.class),
                        examples = @ExampleObject(name = "request",
                                value = MapActiveInfoProductControllerExamples.PRODUCT_CODE_REQUEST_EXAMPLE))))
@ApiResponses({
