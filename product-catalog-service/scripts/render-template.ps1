...
$placeholderPackageRoot = Join-Path (Join-Path $javaRoot "com.viettel.bccs") "productcatalog"
...
foreach ($file in $files) {
    if ($textExtensions -contains $file.Extension) {
        ...
        $content = $content.Replace("product-catalog-service", $ServiceName)
        $content = $content.Replace("com.viettel.bccs", $BasePackage)
        $content = $content.Replace("productcatalog", $ServicePackage)
        $content = $content.Replace("product-catalog-service", $ServiceName)
        $content = $content.Replace("com.viettel.bccs.productcatalog", $packageName)
        $content = $content.Replace("ProductCatalogServiceApplication", $applicationClassName)
        $content = $content.Replace("ProductCatalogServiceApplication", $applicationClassName)
...
$templateApplicationFiles = @(
    (Join-Path $renderedPackageRoot "ProductCatalogServiceApplication.java"),
    (Join-Path $renderedPackageRoot "ProductCatalogServiceApplication.java")
)
