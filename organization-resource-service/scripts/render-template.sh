...
PLACEHOLDER_PACKAGE_ROOT="$OUTPUT_PATH/src/main/java/com.viettel.bccs/organization"
...
\) -print0 | while IFS= read -r -d '' file; do
  sed -i.bak \
    ...
    -e "s|organization|$SERVICE_PACKAGE|g" \
    -e "s|organization-resource-service|$SERVICE_NAME|g" \
    -e "s|com.viettel.bccs.organization|$PACKAGE_NAME|g" \
    -e "s|OrganizationResourceServiceApplication|$APPLICATION_CLASS_NAME|g" \
    -e "s|OrganizationResourceServiceApplication|$APPLICATION_CLASS_NAME|g" \
    "$file"
  rm -f "$file.bak"
done
for template_application_file in \
  "$RENDERED_PACKAGE_ROOT/OrganizationResourceServiceApplication.java" \
  "$RENDERED_PACKAGE_ROOT/OrganizationResourceServiceApplication.java"; do
