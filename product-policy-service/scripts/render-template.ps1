...
$placeholderPackageRoot = Join-Path (Join-Path $javaRoot "com.viettel.bccs") "policy"
...
foreach ($file in $files) {
    if ($textExtensions -contains $file.Extension) {
        ...
        $content = $content.Replace("product-policy-service", $ServiceName)
        $content = $content.Replace("com.viettel.bccs", $BasePackage)
        $content = $content.Replace("policy", $ServicePackage)
        $content = $content.Replace("product-policy-service", $ServiceName)
        $content = $content.Replace("com.viettel.bccs.policy", $packageName)
        $content = $content.Replace("ProductPolicyServiceApplication", $applicationClassName)
        $content = $content.Replace("ProductPolicyServiceApplication", $applicationClassName)
...
$templateApplicationFiles = @(
    (Join-Path $renderedPackageRoot "ProductPolicyServiceApplication.java"),
    (Join-Path $renderedPackageRoot "ProductPolicyServiceApplication.java")
)
