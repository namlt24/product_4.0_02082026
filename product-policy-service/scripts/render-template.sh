...
PLACEHOLDER_PACKAGE_ROOT="$OUTPUT_PATH/src/main/java/com.viettel.bccs/policy"
...
\) -print0 | while IFS= read -r -d '' file; do
  sed -i.bak \
    -e "s|product-policy-service|$SERVICE_NAME|g" \
    -e "s|com.viettel.bccs|$BASE_PACKAGE|g" \
    -e "s|policy|$SERVICE_PACKAGE|g" \
    -e "s|product-policy-service|$SERVICE_NAME|g" \
    -e "s|com.viettel.bccs.policy|$PACKAGE_NAME|g" \
    -e "s|ProductPolicyServiceApplication|$APPLICATION_CLASS_NAME|g" \
    -e "s|ProductPolicyServiceApplication|$APPLICATION_CLASS_NAME|g" \
    "$file"
  rm -f "$file.bak"
done
for template_application_file in \
  "$RENDERED_PACKAGE_ROOT/ProductPolicyServiceApplication.java" \
  "$RENDERED_PACKAGE_ROOT/ProductPolicyServiceApplication.java"; do
